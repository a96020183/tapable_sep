...
**開啟 TapAble 後** —— 手機掛在胸前，雙手留給手杖和平衡。一出電梯，胸前鏡頭在手機上即時辨識，語音提示前方路徑暢通、機台在哪個方向，步伐明顯變快、不怕撞到東西。接近機台時系統提示已抵達（demo 以視覺標記確認），並口述感應貼紙的位置。手機往貼紙一碰 NFC，相機立刻關閉（不耗電、不會拍到別人的個資），手機跳出這台機器的操作介面；用平常就習慣的滑動與語音報讀選「在學證明」、確認，再依語音引導完成機台上的實體步驤，文件印出來。
