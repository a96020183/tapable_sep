...
| 評分項 | 本作品的證據 | 位置 |
|---|---|---|
| **問題定義與影響力 35%** | 逾 2.8 萬台 ATM 僅約 3% 有語音、逾 1/3 語音機失效（公視／身心障礙聯盟）；硬體改造路線的經濟結構不成立；五個操作斷點對照；三位視障者訪談（滿意度 4.67/5）與「想去領錢就去領錢」；電梯口到印出文件的實測情境；銀行／醫院付費、視障者永久免費的商業模式與合規動機 | 「為什麼做這個」「為什麼是手機」「五個實際斷點」「實測情境」「商業模式」「參考來源」|
| **技術實作 30%** | 端側電腦視覺三層（COCO 偵測 → 機台外觀 kNN 比對 → 視覺標記確認）、事件引擎（優先序、逼近偵測、多人去重）；語音輸入四步驗＋覆誦確認；規則式意圖解析＋**選用 LLM 意圖解析**（無 key 自動退回）；Service Worker 離線（18MB 模型預快取）；黑屏防窺＋雙重回饋；Playwright 回歸 37＋29 項與 vision 五支 | `index.html`、`vision/`、`tests/`、「這裡面什麼是真的」|
| **成果展示 20%** | 一個入口頁看全部；60 秒試玩導引；手機與投影兩張截圖；`?stage=1` 手機外框投影模式；2 分鐘影片（見「兩分鐘看懂」）| `start/`、本頁上方 |
| **開源品質 15%** | MIT；第三方授權全文與 NOTICE；`tests/README` 一行指令可重現；`tools/build-kiosk-refs.js` 產生流程；`docs/kiosk-flow.md`（去識別實地流程）、`docs/field-test-protocol.md`（實測方法與限制）；未實作項目逐條誠實標示 | `LICENSE`、`THIRD_PARTY_LICENSES.md`、`tests/`、`tools/`、`docs/` |
...
`vision/index.html`（相機與語音需 HTTPS 或 localhost）。無後端、無資料庫；預設零外部 API 呼叫（本作品程式碼不發任何外部請求；瀏覽器內建語音辨識服務除外；僅在使用者自行貼入 LLM key 時才會呼叫該服務）。
...
模型與函式庫皆隨頁面自帶，預設執行期零外部網路請求（本作品程式碼本身；瀏覽器內建語音辨識除外）（選用的 LLM 意圖解析除外，需使用者自備 key）；影像僅於裝置端記憶體即時處理，不儲存、不上傳。
